    ...
    def push(self, p1, p2, log_p1, log_p2, aij, aji):
        ...
        if aij:
            if self.sg:
                ...
                p1_to_use = p1
            minus_dkl = tf.tensordot(p1_to_use, log_p2, axes=[[0, 1], [0, 1]]) / tf.cast(tf.shape(p1)[0], p1.dtype)
            loss += minus_dkl * aij
        if aji:
            if self.sg:
                ...
                p2_to_use = p2
            minus_dkl = tf.tensordot(p2_to_use, log_p1, axes=[[0, 1], [0, 1]]) / tf.cast(tf.shape(p1)[0], p1.dtype)
            loss += minus_dkl * aji
        return loss
...
class HarmonicContrastive(tf.keras.losses.Loss):
    def __init__(self, eps=None, n=1, **kwargs):
    ...
    def call(self, y_true, y_pred):
        ...
        sim = (dist_sq / normalization_factor) ** (-self.n)
