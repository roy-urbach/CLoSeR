def linear_head_eval(train_embd, svm=True):
    if svm:
...
def classify_head_eval(dataset, m=lambda x: x.reshape(x.shape[0], -1), pca=False, linear=True, samples=0, k=10, **kwargs):
  ...
  if linear:
    model = linear_head_eval(train_embd, **kwargs)
  else:
    from sklearn.neighbors import KNeighborsClassifier
    model = KNeighborsClassifier(k)
    model.fit(train_embd, y_train.flatten()[inds])
  print(f"Train acc: {model.score(train_embd, y_train.flatten()[inds]):.5f}; Test acc: {model.score(m(x_test), y_test.flatten()):.5f}")