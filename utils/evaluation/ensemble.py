...
class EnsembleModel:
    ...
    def fit_with_validation(self, X_train, y_train, X_val, y_val, X_test, y_test,
        ...
        if not self.CS_models:
            if self.is_dct:
                self.scores_val = {k: None for k in X_train}
                self.scores_train = {k: None for k in X_train}
                self.scores_test = {k: None for k in X_train}
                self.best_CS = {k: None for k in X_train}
                for C in self.CS:
                    print(f"{C=}")
                    if C == 0:
                        ...
                        self.base_model.C = C
                    self.fit(X_train, y_train, CS={k: C for k in X_train}, individual_ys=individual_ys)
                    self.CS_models.append(self.models)
                    for k, model in self.models.items():
                        model_val_score = model.score(X_val[k], y_val[k] if individual_ys else y_val)
                        if self.scores_val[k] is None or model_val_score > self.scores_val[k]:
                            self.scores_val[k] = model_val_score
                            self.best_CS[k] = C
                            self.scores_test[k] = model.score(X_test[k], y_test[k] if individual_ys else y_test)
                            self.scores_train[k] = model.score(X_train[k], y_train[k] if individual_ys else y_train)
                self.models = {k: self.CS_models[np.where(self.CS == C)[0][0]][k] for k, C in self.best_CS.items()}
            else:
        ...
        if voting_method is not None:
            ...
            for C in self.CS:
                cur_CS = {k: C for k in self.models} if self.is_dct else [C] * len(self.models)
                ens_score = self.score(X_val, y_val, CS=cur_CS, voting_method=voting_method)
                if ensemble_score_val is None or ens_score > ensemble_score_val:
                    ensemble_score_val = ens_score
                    ensemble_score_test = self.score(X_test, y_test, voting_method=voting_method, CS=cur_CS)
                    ensemble_score_train = self.score(X_train, y_train, voting_method=voting_method, CS=cur_CS)
    ...
    def fit(self, X_train, y_train, CS=None, individual_ys=False):
        ...
        if self.is_dct:
            ...
            for k, X_train_single in counter(X_train.items()):
                ...
                if CS is not None:
                    ...
                    if C == 0:
                        ...
                        self.base_model.penalty = 'l2'
                        self.base_model.C = C
                self.models[k] = model.fit(X_train_single, y_train[k] if individual_ys else y_train)
        else:
            self.models = []
            from sklearn.base import clone as sklearn_clone
    ...
    def predict(self, X, voting_method=EnsembleVotingMethods.ArgmaxMeanProb, CS=None):
        if self.is_dct:
            models = self.models if CS is None else {k: self.CS_models[np.where(self.CS == C)[0][0]][k] for k, C in CS.items()}
            probs = np.stack([getattr(models[k], self.get_score)(X[k]) for k in sorted(list(X.keys()))], axis=-1)
        else:
